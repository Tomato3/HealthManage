...
public class ChoosePrescriptionAdapter extends BaseQuickAdapter<PrescriptionResponse.DataBean, BaseViewHolder> {
    private Context context;
    private DrugsAdapter drugsAdapter;
    private RecyclerView recyclerView;
    public ChoosePrescriptionAdapter(Context context, @Nullable List<PrescriptionResponse.DataBean> data) {
    ...
    protected void convert(BaseViewHolder helper, PrescriptionResponse.DataBean item) {
        ...
        helper.getView(R.id.rbt_choose_model).setVisibility(View.VISIBLE);
        helper.addOnClickListener(R.id.rbt_choose_model);
        helper.setChecked(R.id.rbt_choose_model,item.isSelect() ? true : false);
        helper.setBackgroundRes(R.id.constraintLayout_model,item.isSelect() ? R.drawable.bg_shape_choose_model : R.drawable.bg_shape_solid_task_radiu_5);
