...
public final class BadgeButton extends TextView {
    ...
    public BadgeButton(Context context, AttributeSet attrs, int defStyle) {
        ...
        int badgeText = a.getInt(R.styleable.BadgeButton_btnBadgeText, 0);
    ...
    public BadgeButton setBadgeText(String text) {
        mBadgeDrawable.setText(Integer.parseInt(text));
        return this;
    }
    ...
    private static class BadgeDrawable extends GradientDrawable {
        ...
        void layout(int x, int y, int max) {
            Rect rect = getBounds();
            rect.offsetTo(Math.min(x, max - rect.width() - (int) (0.2f * mHeight)), Math.max(0, 5));
            setBounds(rect);
        }
        ...
        public void draw(Canvas canvas) {
            ...
            super.draw(canvas);
            if (TextUtils.isEmpty(String.valueOf(mText))) {
                return;
            }
            if (Integer.parseInt(mText) > 100) {
                mText = "99+";
            }
            canvas.drawText(String.valueOf(mText), getBounds().exactCenterX(),
                    getBounds().exactCenterY() - (mPaint.descent() + mPaint.ascent()) / 2, mPaint);
        }
    }
